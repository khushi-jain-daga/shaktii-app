const CACHE_NAME = 'pwn-shakti-pkap-workflow-v22';
const APP_SHELL = [
  '/',
  '/index.html',
...
self.addEventListener('fetch', (event) => {
  ...
  if (event.request.mode === 'navigate') {
    event.respondWith(fetch(event.request).then((response) => response.ok ? response : fetch('/')).catch(() => caches.match('/index.html')));
    return;
  }
